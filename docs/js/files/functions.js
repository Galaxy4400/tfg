...
if (iconMenu != null) {
	let delay = 500;
	let menuBody = document.querySelector(".header__actions");
...
function popup_open(item, video = '') {
	...
	if (curent_popup && unlock) {
		...
		if (!document.querySelector('.header__actions._active')) {
			body_lock_add(500);
		}
...
function popup_close(item, bodyUnlock = true) {
	if (unlock) {
		...
		if (!document.querySelector('.header__actions._active') && bodyUnlock) {
